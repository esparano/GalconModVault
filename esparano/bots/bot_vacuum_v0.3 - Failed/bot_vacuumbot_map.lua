...
function bot_vacuumbot_calculate_estimated_time_to_capture(user)
    ...
    -- Get planets to calculate estimated time to capture
    local planets = bot.map.planets
    ...
    ---[[
    -- get initial capture time estimate
    ...
    --]]
    ---[[
    -- get refined capture time estimate
    for _i, p_n in pairs(planets_to_check) do
        local p = planets[p_n]
        p.time_to_capture = estimated_time_to_capture(user, p)
        print("n: " .. p.n .. ", time_to_capture: " .. bot.map.planets[p.n].time_to_capture .. ", busy_until: " .. bot.map.planets[p.n].busy_until)
       
        for _j, p_j in pairs(planets_to_check) do
            local p2 = planets[p_j]
            --p2.time_to_capture = initial_time_to_capture(user, p2)
            --p2.busy_until = p2.time_to_capture    
        end
        -- time to capture IF THIS WERE THE VERY NEXT ONE TO BE ATTACKED
        --p.busy_until = p.time_to_capture
    end
    for _i, p in pairs(planets) do
        p.busy_until = p.time_to_capture
...
function bot_vacuumbot_update_incoming_fleets(user)
    ...
    for _i, planet in pairs(planets) do
        ...
        for _j, fleet in ipairs(incoming_fleets) do
    ...
    for _i,source in pairs(bot.map.planets) do
        --if not g2.item(source.owner_n).user_neutral then -- DON'T COMPUTE TUNNELS STARTING AT NEUTRAL PLANETS.... TEMPORARY
            ...
            for _j,dest in pairs(bot.map.planets) do
                ...
                if source.tunnel[dest.n] == nil then print("NIL TUNNEL WTF") end
                source.tunnel_dist[dest.n] = _bot_vacuumbot_compute_tunnel_dist(source, dest)
                if source.tunnel[dest.n] == nil then print("NIL TUNNEL DIST WTF") end
            end
        --end
    end
end
