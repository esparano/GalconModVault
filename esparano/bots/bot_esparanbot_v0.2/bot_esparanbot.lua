...
function bot_esparanbot(user)
    ...
    for _i, target in pairs(start_s.planets) do
        ...
        for _i, action in pairs(actions) do
            ...
            --print("sim_eval: " .. math.floor(result_eval_using_full_sim) .. " result_eval: " .. math.floor(result_eval) .. " base_eval: " .. math.floor(base_eval))
            --]]
            ...
            if action.type == "redirect" then
                ...
                local result_eval_using_full_sim = bot_esparanbot_eval1(e, user)
                print("sim_eval: " .. math.floor(result_eval_using_full_sim) .. " result_eval: " .. math.floor(result_eval) .. " base_eval: " .. math.floor(base_eval)
                        .. " old_target: " .. math.floor(action.old_target.ships_value) .. " new_target: " .. math.floor(action.new_target.ships_value) .. " fleet_ships: " .. math.floor(action.fleet.fleet_ships))
            end
            --]]
            
            local return_on_investment = (result_eval - base_eval)
            if math.floor(return_on_investment) > math.floor(best_return) then
