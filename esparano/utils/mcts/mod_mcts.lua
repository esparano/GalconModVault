...
local function backPropagate(node, terminalState)
    while node do
        local parentsStatesCurrentAgent = node:getPreviousAgent()
        local reward = parentsStatesCurrentAgent:getRewardFromTerminalState(terminalState)
