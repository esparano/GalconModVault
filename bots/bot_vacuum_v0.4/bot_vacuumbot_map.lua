...
function bot_vacuumbot_get_map(user)
    local map = {}
    ...
    for _i, p in pairs(map.planets) do
        if p.owner_n == user.n then 
            ...
            p.time_to_capture = 2000000
            p.busy_until = 2000000
        end
    end 
...
function bot_vacuumbot_calculate_initial_time_to_capture(user)
    local bot = BOT_VACUUMBOT[user.n]
    local planets = bot.map.planets
    -- initial capture time prediction
    for _i, p in pairs(planets) do
...
function bot_vacuumbot_calculate_estimated_time_to_capture(user)
    ...
    table.sort(planets_to_check, function(p1_n,p2_n) 
        ...
        if (prod_to_ships(p1.ships_production) * bot.horizon - p1.sort_ships_value) / (p1.sort_ships_value + 1) 
            > (prod_to_ships(p2.ships_production) * bot.horizon - p2.sort_ships_value) / (p2.sort_ships_value + 1)  then 
            return true 
        end 
    end)
    
    -- refined capture time prediction
...
function bot_vacuumbot_calculate_better_estimated_time_to_capture(user)
    local bot = BOT_VACUUMBOT[user.n]
    local planets = bot.map.planets
    ...
    table.sort(planets_to_check, function(p1_n,p2_n) 
        local p1 = planets[p1_n]
        local p2 = planets[p2_n]
        if (prod_to_ships(p1.ships_production) * (bot.horizon - p1.time_to_capture) - p1.sort_ships_value) / (p1.sort_ships_value + 1) 
            > (prod_to_ships(p2.ships_production) * (bot.horizon - p2.time_to_capture) - p2.sort_ships_value) / (p2.sort_ships_value + 1)  then 
            return true 
        end 
    end)
    ...
    for _i, p_n in pairs(planets_to_check) do
        local p = planets[p_n]
        if g2.item(p.owner_n).user_neutral then
            p.time_to_capture = better_estimated_time_to_capture(user, p)
        end
    end
end
...
function _bot_vacuumbot_compute_tunnel(user, source, dest)
    ...
    for _i,m in ipairs(source.neighbors) do
        if m == dest.n then break end -- only check planets BETWEEN source and dest
        local middle = bot.map.planets[m]
        if source.owner_n == middle.owner_n then --or (g2.item(middle.owner_n).user_neutral and (middle.time_to_capture < bot.horizon and middle.time_to_capture < dest.time_to_capture)) then
            -- is tunneling faster?
