...
function bot_vacuumbot(user)
    ...
    bot_vacuumbot_update_incoming_fleets(user)  -- estimation changes incoming fleets
    -- TEST TUNNELS
    --[[  
    local path = ""
    ...
    for _i,p in pairs(bot.map.planets) do
        ...
        --print("n: " .. p.n .. ", est_ships: " .. p.est_ships_value) 
    end
    ...
    table.sort(targets, function(p1,p2) 
        ...
        ---[[
        if (prod_to_ships(p1.ships_production) * (bot.horizon - planet1.time_to_capture) - planet1.ships_value) / (planet1.ships_value + 1) 
            > (prod_to_ships(p2.ships_production) * (bot.horizon - planet2.time_to_capture) - planet2.ships_value) / (planet2.ships_value + 1)  then 
            return true 
        end 
        --]]
        --[[
                ...
                if p.owner_n == user.n then
                    ...
                    if p.est_ships_value > amount_left then -- there are enough stored ships in this planet to capture 
                        ...
                        if p.ships_value < amount_left then 
                            p.total_incoming_fleets = p.total_incoming_fleets - (amount_left - p.ships_value)
                        end
                        p.est_ships_value = p.est_ships_value - amount_left
                        break
                    else
                        local ships_sent = send_exact(user, g2.item(p.n), g2.item(tunnel_alias.n), p.est_ships_value)
                        amount_left = amount_left - ships_sent - p.total_incoming_fleets -- account for the fact that the fleets will be there in the future
                        -- use up fleets
                        p.total_incoming_fleets = 0
                        --p.total_incoming_fleets = p.est_ships_value - p.ships_value
                        p.est_ships_value = 0
                    end
                end
        ...
        for _j, p_n in ipairs(planets_to_check) do
            ...
            if p.owner_n == user.n then
                ...
                if p.est_ships_value > amount_left then -- there are enough stored ships in this planet to capture 
                    ...
                    local ships_sent = p.ships_value
                    amount_left = amount_left - ships_sent - p.total_incoming_fleets -- account for the fact that the fleets will be there in the future
                    -- use up fleets
                    p.est_ships_value = 0
                    p.total_incoming_fleets = 0
