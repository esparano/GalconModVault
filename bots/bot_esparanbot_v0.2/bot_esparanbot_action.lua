function bot_esparanbot_get_actions(start_s, target, user, map)
    local actions = {}
    -- for each planet near target, try to gather enough ships to take the target
    ...
    for _i, fleet in pairs(start_s.fleets) do
        -- friendly fleet not already headed to target
        if fleet.owner_n == user.n and fleet.fleet_target ~= target.n then
...
function bot_esparanbot_apply_action(s, a)
    if a.type == "redirect" then
        ...
        table.insert(s.incoming_fleets[a.new_target.n], a.fleet)
        for i, f in ipairs(s.incoming_fleets[a.old_target.n]) do
            if f.n == a.fleet.n then
                table.remove(s.incoming_fleets[a.old_target.n], i)
            end
        end        
    elseif a.type == "send" then
...
function bot_esparanbot_return_on_redirect(s, action, horizon, user)
    -- don't redirect to the same planet....
    if action.old_target.n == action.new_target.n then return 0 end
    ...
    for i=1,2 do
        ...
        elseif not g2.item(target.owner_n).user_neutral then
            baseline = baseline - target_ships_value
        end
    end
